...
if isequal(id_char,']')
    ...
    PT_arr = strread(line,'%f','delimiter',',');  % <-- %f supports scientific notation
    [r,~] = size(PT_arr);
    n = floor(r/3);
...
function [TR,n] = read_TR(line)
    %reads in triangle indices and returns a N by 3
    line = strtrim(line);                % remove leading/trailing spaces
    TR_arr = strread(line,'%f','delimiter',',');  % %f supports scientific notation
    %delete negative ones
    TR_arr(TR_arr < 0) = [];
    [r,~] = size(TR_arr);
    n = floor(r/3);
    TR_temp = reshape(TR_arr,3,n);
    TR = TR_temp';
